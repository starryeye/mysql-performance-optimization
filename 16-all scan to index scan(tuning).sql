...
-- Question, all type scan.. (모든데이터 접근후 sort 과정도 해주는중)
...
-- index type scan
explain analyze SELECT * from users
	order by salary -- 인덱스를 활용했기 때문에 sort 작업을 할 필요가 없어졌고 조회해야할 데이터도 100개로 확 줄었다. (index type scan) 
	LIMIT 100; -- limit 이 없으면 옵티마이저가 all type scan 을 해버린다. (optimizer may not always use an index 참고)